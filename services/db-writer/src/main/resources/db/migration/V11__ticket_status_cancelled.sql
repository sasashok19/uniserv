-- Add 'cancelled' to the tickets.status CHECK (Feature 21): an admin-only
-- terminal state for a ticket that should never have existed as work — a
-- duplicate the citizen confirmed, a test row, a mistaken or withdrawn
-- complaint. Deliberately distinct from 'closed': closed means the work was
-- done, cancelled means there was no work to do, and reporting must be able
-- to tell those apart (a cancelled ticket is neither an open backlog item nor
-- a resolved one).
--
-- SQLite cannot alter a CHECK constraint, so rebuild the table in place
-- (standard 12-step): new table with the widened CHECK, copy, swap, recreate
-- indexes. Same shape as V9, which added 'pending_customer' the same way.
CREATE TABLE tickets_new (
  ...
  status           TEXT DEFAULT 'open'
                   CHECK(status IN ('open','assigned','in_progress','pending_customer','resolved','closed','reopened','cancelled')),
  category         TEXT,
  subcategory      TEXT,
  priority_score   REAL,
