...
def fields_for_channel(tenant_config: Optional[dict], channel: str) -> list[dict]:
    ...
    return [f for f in fields if f.get("key") in FIELD_CATALOG]
...
def extract_configured_fields(
    ...
    for fc in field_configs:
        ...
        if known and profile_field and known.get(profile_field):
            ...
            continue
        raw_value = FIELD_CATALOG[key]["extract"](text)
        if raw_value is None:
            result[key] = {"value": None, "valid": True, "source": None}
        else:
            result[key] = {
                "value": raw_value, "valid": FIELD_CATALOG[key]["validate"](raw_value), "source": "extracted",
            }
    return result


def missing_fields(extracted: dict, field_configs: list[dict], declared_anonymous: bool) -> list[str]:
    ...
    for fc in field_configs:
        ...
        if value is None:
            if required:
                missing.append(FIELD_CATALOG[key]["label"])
        elif not entry.get("valid", True):
            missing.append(f"a valid {FIELD_CATALOG[key]['label']} (the one you sent doesn't look right)")
    return missing


def build_identity_request_message(
    field_configs: list[dict], channel: str, channel_identity_verified: bool,
    missing: list[str], is_first_ask: bool,
) -> str:
