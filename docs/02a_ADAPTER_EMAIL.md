...
- **Architecture pivot — webhook, not IMAP:** IMAP polling was removed entirely (no `EmailAdapter.poll()`, no `EMAIL_IMAP_*` env vars, no `/api/v1/internal/adapters/email/poll` endpoint). Make.com watches the mailbox (Gmail/Outlook) and POSTs the parsed message to `POST /api/v1/webhooks/email`, guarded by an `X-Webhook-Secret` header (`EMAIL_WEBHOOK_SECRET`, dev default `dev-webhook-secret`). The gateway backfills `id`/`timestamp`/`receivedAt`/`traceId`, forces `channelIdentity = {type:"email", verified:false}` per the 02f contract regardless of what the payload says, validates the result via `EventValidator`, and publishes to Valkey off the request thread so the HTTP response returns immediately (200 `{"received":true}`; 401 on bad secret; 400 `{"valid":false,"errors":[...]}` on a malformed payload). SMTP outbound (`sendReply` / `test-send`) is unchanged.
